"""Unittesting for payslip module"""
...
class TestPayslip(unittest.TestCase):
    ...
    def test_init(self):
        for (key, salary) in self.salary.items():
            if key in self.salary_names:
                ...
                self.assertRaises(TypeError, Payslip, self.salary['string'])
            elif salary == -300:
                self.assertRaises(ValueError, Payslip, self.salary['negative'])

    def test_tax_calculator(self):
        for salary_name in self.salary_names:
            pspf = Payslip(self.salary[salary_name])
    ...
    def test_national_insurance_calculator(self):
        for salary_name in self.salary_names:
            pspf = Payslip(self.salary[salary_name])
            self.assertEqual(
                self.exp_ni[salary_name],
                pspf.national_insurance_calculator()
            )
    ...
    def test_student_loan_calculator(self):
        for salary_name in self.salary_names:
            pspf = Payslip(self.salary[salary_name])
            self.assertEqual(
                self.exp_sl[salary_name],
                pspf.student_loan_repayment_calculator(),
            )
    ...
    def test_autoenrolement_pension(self):
        for salary_name in self.salary_names:
            pspf = Payslip(self.salary[salary_name])
            self.assertEqual(
                self.exp_ae_pension[salary_name],
                pspf.autoenrolement_pension(),
            )
    ...
    def test_compound_interest_calculator(self):
        for salary_name in self.salary_names:
            pspf = Payslip(self.salary[salary_name])
            self.assertEqual(
                self.exp_interest['compound'],
                pspf.compound_interest_calculator(
    ...
    def test_simple_interest_calculator(self):
        for salary_name in self.salary_names:
            pspf = Payslip(self.salary[salary_name])
            self.assertEqual(
                self.exp_interest['simple'],
                pspf.simple_interest_calculator(
                    self.interest['principal'],
                    self.interest['rate'],
                    self.interest['time'],
                )
